...
import { createJob, getJobs, API_BASE } from "@/lib/api";
...
const PRESETS = ["standard", "viral", "meme", "cinematic"] as const;
...
export default function Home() {
  ...
  return (
    <div style={{ padding: 20, fontFamily: "system-ui" }}>
      <h1>VideoMRP v2</h1>
      <p style={{ opacity: 0.8 }}>
        Nhập URL trực tiếp tới file video (mp4/mov/...) hoặc dùng upload API (tuỳ bạn mở rộng UI sau).
      </p>
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 10, maxWidth: 720 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <label>
            Target lang:
            <input value={targetLang} onChange={(e) => setTargetLang(e.target.value)} style={{ marginLeft: 8 }} />
          </label>
          ...
          <label>
            ...
            <input
              type="number"
              value={clipSeconds}
              onChange={(e) => setClipSeconds(parseInt(e.target.value || "0", 10))}
          ...
          <label>
            Preset:
            <select value={preset} onChange={(e) => setPreset(e.target.value as any)} style={{ marginLeft: 8 }}>
        ...
        <button disabled={!canSubmit || busy}>{busy ? "Processing..." : "Create & Start"}</button>
        ...
        {jobs.map((j) => (
          <div key={j.id} style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
              <b>{j.title}</b>
              <span>{j.status} • {Math.round(j.progress)}%</span>
            </div>
            <div style={{ opacity: 0.8 }}>Step: {j.current_step}</div>
            {j.error_message && <div style={{ color: "crimson" }}>{j.error_message}</div>}
            {j.processed_filename && (
              <a href={`${API_BASE}/processed/${j.processed_filename}`} target="_blank" rel="noreferrer">
